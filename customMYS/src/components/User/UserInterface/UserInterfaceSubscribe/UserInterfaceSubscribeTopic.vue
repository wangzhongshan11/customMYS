<template>
  <div style="background-color:#dddddd ;padding-top:0px;">
    <div
      ...
      v-for="(item,index) in interfaceUser.subscribeTopics"
      :key="index"
    >
      <div>
        <el-row>
...
export default {
  mounted() {    
    ...
    this.$nextTick(() => {
      ...
      if (isNotNullORBlank(this.interfaceUser.subscribeTopics)) {
      this.interfaceUser.subscribeTopics.forEach((item, index) => {
        // console.log(index);
    ...
    subscribeTopic(index) {
      ...
      if (
        this.$store.state.currentUser.subscribeTopics.some((item1, index1) => {
          return item1.id == this.interfaceUser.subscribeTopics[index].id;
        })
      ) {
        console.log("取消关注");
        deleteRequest(
          "/user/cancelSub/topic/" +
            ...
            "/" +
            _this.interfaceUser.subscribeTopics[index].id
        ).then(
          function (msg) {
            if (msg.status !== 200) {
        ...
        postRequest(
          ...
          function (msg) {
            if (msg.status !== 200) {
              ...
              var currentUser = _this.$store.state.currentUser;
              currentUser.subscribeTopics.push(_this.interfaceUser.subscribeTopics[index]);
              _this.$store.dispatch("setCurrentUser", currentUser);
