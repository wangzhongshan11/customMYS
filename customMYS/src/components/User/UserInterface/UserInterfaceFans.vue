  ...
  <div class="user-fans-wrap">
    ...
    <div class="fans-main">
      <div style="background-color:#fff;">
        <div v-for="(item,index) in interfaceUser.fans" :key="index">
          <el-row>
            <el-col :span="2" :offset="0">
              <div class="flex-div-end-wrap" style="height:100px;cursor:pointer;" @click="goUserInterface(item.aid)">
                <img
                  class="user-avatar"
                  :src="item.userface"
                  alt="wzs"
            ...
            <el-col :span="20" :offset="0">
              <div style="height:100px;padding-left:10px;padding-top:25px;margin-bottom:-25px;cursor:pointer;" @click="goUserInterface(item.aid)">
...
export default {
  mounted() {
    ...
    this.$nextTick(() => {
      if (isNotNullORBlank(this.interfaceUser.fans)) {
        this.interfaceUser.fans.forEach((item, index) => {
          console.log(index);
          if (
    ...
    subscribeUser(ind) {
      ...
      if (
        this.$store.state.currentUser.subscribes.some((item, index) => {
          return item.bid == this.interfaceUser.fans[ind].aid;
        })
      ) {
        console.log("取消关注");
        deleteRequest(
          "/user/cancelSub/" +
            ...
            "/" +
            _this.interfaceUser.fans[ind].aid
        ).then(
          function (msg) {
            if (msg.status !== 200) {
              ...
              currentUser1.subscribes.forEach((item, index) => {
                if (item.bid == _this.interfaceUser.fans[ind].aid) {
