<template>
  <div style="background-color:#dddddd;padding-top:0px;">
    <div style="background-color:#fff;padding-left:3%;padding-right:2%;">
      <div v-for="(item,index) in currentUser.subscribes" :key="index">
        <el-row>
      ...
      <div >
        <el-row>
          ...
          <el-col :span="20" :offset="0">
            <div style="height:100px;padding-left:10px;padding-top:25px;margin-bottom:-25px;cursor:pointer;" >
              <div class="flex-div-start-wrap">
                <span>若鸿似鹄</span>
              </div>
              <div class="flex-div-start-wrap" style="margin-top:10px;">
...
export default {
  mounted() {
    ...
    // console.log(this.currentUser.subscribes.length);
    this.$nextTick(() => {
      if (isNotNullORBlank(this.currentUser.subscribes)) {
        this.currentUser.subscribes.forEach((item, index) => {
          // console.log(index);
          if (
    ...
    subscribeUser(ind) {
      ...
      if (
        this.$store.state.currentUser.subscribes.some((item, index) => {
          return item.bid == this.currentUser.subscribes[ind].bid;
        })
      ) {
        console.log("取消关注");
        deleteRequest(
          "/user/cancelSub/" +
            ...
            "/" +
            _this.currentUser.subscribes[ind].bid
        ).then(
          function (msg) {
            if (msg.status !== 200) {
              ...
              currentUser1.subscribes.forEach((item, index) => {
                if (item.bid == _this.currentUser.subscribes[ind].bid) {
