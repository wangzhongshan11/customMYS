  ...
  <div>
    <div class="page-container-wrap">
      ...
      >
        ...
        >
          <template v-for="(item,index) in this.$router.options.routes[18].children">
            <el-menu-item
              :index="item.path"
              :key="index"
              style="font-size:17px;"
              @click="dabieyeRouteDirect(index)"
              class="bbb-routes-wrap-suspension"
...
export default {
  ...
  mounted() {
    this.articles = this.$store.state.BBBArticles;
    console.log("DicussRegionBBB");
  ...
  methods: {
    ...
    dabieyeRouteDirect(ind) {
      ...
      document
        .getElementsByClassName("bbb-routes-wrap-suspension")
        [ind].click();
      document.getElementsByClassName("bbb-routes-wrap")[ind].click();
    ...
    signInForum() {
      var _this = this;
      putRequest(
        "/user/signIn/" + _this.$store.state.currentUser.id + "/" + "大别野"
      ).then(
        (resp) => {
          if (resp.status == 200) {
