<template>
  <div>
    <topicPopup ref="dailyPopupIndex" :topic="dabieyeACGTopics"></topicPopup>
    <div
      ...
      >
        <div class="swiper-wrapper">
          <div
            ...
            v-for="(item,index) in dabieyeACGTopics"
            :key="index"
            style="width:auto;margin-right:0px;"
            @click="clickTopicSus(index)"
          >
            <span
              class="swiper-slide-ACG-suspension"
      ...
      >
        ...
        >
          ...
          <span class="flex-div-end-wrap" style="height:23px;">
            ...
            <span style="margin-left:12%;" class="subscribe-article-wrap-ACG">
...
export default {
  ...
  mounted() {
    this.loadPlateBlogs("ACG");
    this.loadPlateTopics("ACG");
  ...
  methods: {
    iniSubscribeButton() {
      if (isNotNullORBlank(this.articles)) {
        this.articles.forEach((item, index) => {
          if (
            ...
            document.getElementsByClassName("subscribe-article-wrap-ACG")[
              index
            ].style.color = "#ff9900";
          }
        });
      ...
      if (this.chosenTopicIndex == 0) {
        ...
        document.getElementsByClassName("swiper-slide-ACG")[0].style.color =
          "#fff";
    ...
    clickTopic(ind) {
      var _this = this;
      if (this.dabieyeACGTopics[ind].topicname !== "全部") {
        this.loadTopicBlogs(this.dabieyeACGTopics[ind].topicname);
      ...
      var swiperSlideDailys = Array.from(
        document.getElementsByClassName("swiper-slide-ACG")
      );
      var swiperSlideDailySuspensions = Array.from(
        document.getElementsByClassName("swiper-slide-ACG-suspension")
      );
    ...
    initArticles() {
      ...
      if (isNotNullORBlank(stateArticles)) {
        stateArticles.forEach((item, index) => {
          var _this = this;
          if (stateArticles[index].platename == "ACG") {
      ...
      getRequest(url).then(
        (resp) => {
          ...
          if (resp.status == 200) {
            // console.log("=200");
            // console.log(resp.data);
            _this.dabieyeACGTopics = resp.data;
          } else {
            _this.$message({ type: "error", message: "数据加载失败!" });
          }
        },
