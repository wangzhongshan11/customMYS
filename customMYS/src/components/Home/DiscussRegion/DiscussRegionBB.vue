...
<div
      ...
      >
        ...
        >
          <template v-for="(item,index) in this.$router.options.routes[19].children">
            <el-menu-item
              :index="item.path"
              :key="index"
...
export default {
  ...
  methods: {
    ...
    signInForum() {
      var _this = this;
      putRequest("/user/signIn/" + _this.$store.state.currentUser.id + "/" + "崩坏学园2").then((resp) => {
        if (resp.status == 200){
          var currentUser = _this.$store.state.currentUser;
          currentUser.userForums.forEach((item,index) =>  {
            if (item.forumname == "崩坏学园2") {
