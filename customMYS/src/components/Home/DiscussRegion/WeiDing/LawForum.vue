<template>
  <div>
    <topicPopup ref="dailyPopupIndex" :topic="weidingLawForumTopics"></topicPopup>
    <div
      ...
      >
        <div class="swiper-wrapper">
          <div
            ...
            v-for="(item,index) in weidingLawForumTopics"
            :key="index"
            style="width:auto;margin-right:0px;"
            @click="clickTopicSus(index)"
          >
            <span
              class="swiper-slide-lawforum-suspension"
      ...
      >
        ...
        >
          ...
          <span class="flex-div-end-wrap" style="height:23px;">
            ...
            <span style="margin-left:12%;" class="subscribe-article-wrap-lawforum">
...
export default {
  ...
  mounted() {
    this.loadPlateBlogs("律所");
    this.loadPlateTopics("律所");
  ...
  methods: {
    iniSubscribeButton() {
      if (isNotNullORBlank(this.articles)) {
        this.articles.forEach((item, index) => {
          ...
          if (
            ...
            console.log(index);
            console.log(
              document.getElementsByClassName("subscribe-article-wrap-lawforum")
            );
      ...
      if (this.chosenTopicIndex == 0) {
        document.getElementsByClassName(
          "swiper-slide-lawforum"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName("swiper-slide-lawforum")[0].style.color =
          "#fff";
        document.getElementsByClassName(
          "swiper-slide-lawforum-suspension"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName(
          "swiper-slide-lawforum-suspension"
        )[0].style.color = "#fff";
    ...
    clickTopic(ind) {
      var _this = this;
      if (this.weidingLawForumTopics[ind].topicname !== "全部") {
        this.loadTopicBlogs(this.weidingLawForumTopics[ind].topicname);
      ...
      var swiperSlideDailys = Array.from(
        document.getElementsByClassName("swiper-slide-lawforum")
      );
      var swiperSlideDailySuspensions = Array.from(
        document.getElementsByClassName("swiper-slide-lawforum-suspension")
      );
    ...
    initArticles() {
      ...
      if (isNotNullORBlank(stateArticles)) {
        stateArticles.forEach((item, index) => {
          var _this = this;
          if (stateArticles[index].platename == "律所") {
    ...
    subscribeArticle(ind) {
      ...
      var subscribeButton = document.getElementsByClassName(
        "subscribe-article-wrap-lawforum"
      )[ind];
      ...
      getRequest(url).then(
        (resp) => {
          ...
          if (resp.status == 200) {
            // console.log("=200");
            // console.log(resp.data);
            _this.weidingLawForumTopics = resp.data;
          } else {
            _this.$message({ type: "error", message: "数据加载失败!" });
          }
        },
