<template>
  <div>
    <topicPopup ref="dailyPopupIndex" :topic="BBBStrategyTopics"></topicPopup>
    <div
    ...
    >
      ...
      >
        <div class="swiper-wrapper">
          <div
            ...
            v-for="(item,index) in BBBStrategyTopics"
            :key="index"
            style="width:auto;margin-right:0px;"
            @click="clickTopicSus(index)"
          >
            <span
              class="swiper-slide-strategy-suspension"
      ...
      <div
        class="swiper-container swiper-container-bbbimg"
        style="margin-top:10px;width:100%;margin-left:7%;margin-right:7%;"
      >
        <div class="swiper-wrapper">
          <div
            class="swiper-slide flex-div-center-wrap"
            v-for="(item,index) in bbbSwiperImg"
            :key="index"
            style="width:auto;margin-right:-10px;"
            
          >
            <img :src="require('../../../../img/'+item)" alt="wzs" style="height:200px;border-radius:10px;">
          </div>
        </div>
      </div>
    </div>
      ...
      >
        ...
        >
          ...
          <span class="flex-div-end-wrap" style="height:23px;">
            ...
            <span style="margin-left:12%;" class="subscribe-article-wrap-strategy">
...
export default {
  ...
  mounted() {
    this.loadPlateBlogs("攻略");
    this.loadPlateTopics("攻略");
    new Swiper(".swiper-container", {
      slidesPerView: "auto",
      observer: true,
    });
            new Swiper(".swiper-container-bbbimg", {
      slidesPerView: "1.8",
      observer:true
    });
    // this.$nextTick(() => {
  ...
  data() {
    return {
      ...
      articles: [],
      chosenTopicIndex: 0,
      bbbSwiperImg:['布洛妮娅2.jpg','吼姆.jpg','琪亚娜圣诞节.jpg','希儿3.jpg']
    };
  },
  updated() {
    this.$nextTick(() => {
  ...
  methods: {
    iniSubscribeButton() {
      if (isNotNullORBlank(this.articles)) {
        this.articles.forEach((item, index) => {
          ...
          if (
            ...
            console.log(index);
            console.log(
              document.getElementsByClassName("subscribe-article-wrap-strategy")
            );
      ...
      if (this.chosenTopicIndex == 0) {
        document.getElementsByClassName(
          "swiper-slide-strategy"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName("swiper-slide-strategy")[0].style.color =
          "#fff";
        document.getElementsByClassName(
          "swiper-slide-strategy-suspension"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName(
          "swiper-slide-strategy-suspension"
        )[0].style.color = "#fff";
    ...
    clickTopic(ind) {
      var _this = this;
      if (this.BBBStrategyTopics[ind].topicname !== "全部") {
        this.loadTopicBlogs(this.BBBStrategyTopics[ind].topicname);
        this.chosenTopicIndex = ind;
      } else {
        this.loadPlateBlogs("日常");
      }

      var swiperSlideDailys = Array.from(
        document.getElementsByClassName("swiper-slide-strategy")
      );
      var swiperSlideDailySuspensions = Array.from(
        document.getElementsByClassName("swiper-slide-strategy-suspension")
      );
    ...
    initArticles() {
      var stateArticles = this.$store.state.BBBArticles;
      if (isNotNullORBlank(stateArticles)) {
        stateArticles.forEach((item, index) => {
          var _this = this;
          if (stateArticles[index].platename == "攻略") {
    ...
    subscribeArticle(ind) {
      ...
      var subscribeButton = document.getElementsByClassName(
        "subscribe-article-wrap-strategy"
      )[ind];
      ...
      getRequest(url).then(
        (resp) => {
          ...
          if (resp.status == 200) {
            // console.log("=200");
            // console.log(resp.data);
            _this.BBBStrategyTopics = resp.data;
          } else {
            _this.$message({ type: "error", message: "数据加载失败!" });
          }
        },
