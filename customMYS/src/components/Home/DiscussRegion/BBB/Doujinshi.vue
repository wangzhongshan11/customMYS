<template>
  <div>
    <topicPopup ref="dailyPopupIndex" :topic="BBBDoujinshiTopics"></topicPopup>
    <div
      ...
      >
        <div class="swiper-wrapper">
          <div
            ...
            v-for="(item,index) in BBBDoujinshiTopics"
            :key="index"
            style="width:auto;margin-right:0px;"
            @click="clickTopicSus(index)"
          >
            <span
              class="swiper-slide-doujinshi-suspension"
      ...
      >
        ...
        >
          ...
          <span class="flex-div-end-wrap" style="height:23px;">
            ...
            <span style="margin-left:12%;" class="subscribe-article-wrap-doujinshi">
...
export default {
  ...
  methods: {
    iniSubscribeButton() {
      if (isNotNullORBlank(this.articles)) {
        this.articles.forEach((item, index) => {
          ...
          if (
            ...
            console.log(index);
            console.log(
              document.getElementsByClassName("subscribe-article-wrap-doujinshi")
            );
      ...
      if (this.chosenTopicIndex == 0) {
        document.getElementsByClassName(
          "swiper-slide-doujinshi"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName("swiper-slide-doujinshi")[0].style.color =
          "#fff";
        document.getElementsByClassName(
          "swiper-slide-doujinshi-suspension"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName(
          "swiper-slide-doujinshi-suspension"
        )[0].style.color = "#fff";
    ...
    clickTopic(ind) {
      var _this = this;
      if (this.BBBDoujinshiTopics[ind].topicname !== "全部") {
        this.loadTopicBlogs(this.BBBDoujinshiTopics[ind].topicname);
      ...
      var swiperSlideDailys = Array.from(
        document.getElementsByClassName("swiper-slide-doujinshi")
      );
      var swiperSlideDailySuspensions = Array.from(
        document.getElementsByClassName("swiper-slide-doujinshi-suspension")
      );
    ...
    subscribeArticle(ind) {
      ...
      var subscribeButton = document.getElementsByClassName(
        "subscribe-article-wrap-doujinshi"
      )[ind];
      ...
      getRequest(url).then(
        (resp) => {
          ...
          if (resp.status == 200) {
            // console.log("=200");
            // console.log(resp.data);
            _this.BBBDoujinshiTopics = resp.data;
          } else {
            _this.$message({ type: "error", message: "数据加载失败!" });
          }
        },
