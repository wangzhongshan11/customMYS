<template>
  <div>
    <topicPopup ref="dailyPopupIndex" :topic="yuanshenTavernTopics"></topicPopup>
    <div
      ...
      >
        <div class="swiper-wrapper">
          <div
            ...
            v-for="(item,index) in yuanshenTavernTopics"
            :key="index"
            style="width:auto;margin-right:0px;"
            @click="clickTopicSus(index)"
          >
            <span
              class="swiper-slide-tavern-suspension"
      ...
      >
        ...
        >
          ...
          <span class="flex-div-end-wrap" style="height:23px;">
            ...
            <span style="margin-left:12%;" class="subscribe-article-wrap-tavern">
...
export default {
  ...
  mounted() {
    this.loadPlateBlogs("酒馆");
    this.loadPlateTopics("酒馆");
  ...
  methods: {
    iniSubscribeButton() {
      if (isNotNullORBlank(this.articles)) {
        this.articles.forEach((item, index) => {
          ...
          if (
            ...
            console.log(index);
            console.log(
              document.getElementsByClassName("subscribe-article-wrap-tavern")
            );
      ...
      if (this.chosenTopicIndex == 0) {
        ...
        document.getElementsByClassName("swiper-slide-tavern")[0].style.color =
          "#fff";
        document.getElementsByClassName(
          "swiper-slide-tavern-suspension"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName(
          "swiper-slide-tavern-suspension"
        )[0].style.color = "#fff";
    ...
    clickTopic(ind) {
      var _this = this;
      if (this.yuanshenTavernTopics[ind].topicname !== "全部") {
        this.loadTopicBlogs(this.yuanshenTavernTopics[ind].topicname);
      ...
      var swiperSlideDailys = Array.from(
        document.getElementsByClassName("swiper-slide-tavern")
      );
      var swiperSlideDailySuspensions = Array.from(
        document.getElementsByClassName("swiper-slide-tavern-suspension")
      );
    ...
    initArticles() {
      ...
      if (isNotNullORBlank(stateArticles)) {
        stateArticles.forEach((item, index) => {
          var _this = this;
          if (stateArticles[index].platename == "酒馆") {
      ...
      getRequest(url).then(
        (resp) => {
          ...
          if (resp.status == 200) {
            // console.log("=200");
            // console.log(resp.data);
            _this.yuanshenTavernTopics = resp.data;
          } else {
            _this.$message({ type: "error", message: "数据加载失败!" });
          }
        },
