<template>
  <div>
    <topicPopup ref="dailyPopupIndex" :topic="BBSchoolTopics"></topicPopup>
    <div
      ...
      >
        <div class="swiper-wrapper">
          <div
            ...
            v-for="(item,index) in BBSchoolTopics"
            :key="index"
            style="width:auto;margin-right:0px;"
            @click="clickTopicSus(index)"
          >
            <span
              class="swiper-slide-school-suspension"
      ...
      >
        ...
        >
          ...
          <span class="flex-div-end-wrap" style="height:23px;">
            ...
            <span style="margin-left:12%;" class="subscribe-article-wrap-school">
...
export default {
  ...
  mounted() {
    this.loadPlateBlogs("学园");
    this.loadPlateTopics("学园");
  ...
  methods: {
    iniSubscribeButton() {
      if (isNotNullORBlank(this.articles)) {
        this.articles.forEach((item, index) => {
          ...
          if (
            ...
            console.log(index);
            console.log(
              document.getElementsByClassName("subscribe-article-wrap-school")
            );
      ...
      if (this.chosenTopicIndex == 0) {
        ...
        document.getElementsByClassName("swiper-slide-school")[0].style.color =
          "#fff";
        document.getElementsByClassName(
          "swiper-slide-school-suspension"
        )[0].style.backgroundColor = "#000000";
        document.getElementsByClassName(
          "swiper-slide-school-suspension"
        )[0].style.color = "#fff";
    ...
    clickTopic(ind) {
      var _this = this;
      if (this.BBSchoolTopics[ind].topicname !== "全部") {
        this.loadTopicBlogs(this.BBSchoolTopics[ind].topicname);
      ...
      var swiperSlideDailys = Array.from(
        document.getElementsByClassName("swiper-slide-school")
      );
      var swiperSlideDailySuspensions = Array.from(
        document.getElementsByClassName("swiper-slide-school-suspension")
      );
    ...
    initArticles() {
      ...
      if (isNotNullORBlank(stateArticles)) {
        stateArticles.forEach((item, index) => {
          var _this = this;
          if (stateArticles[index].platename == "学园") {
      ...
      getRequest(url).then(
        (resp) => {
          ...
          if (resp.status == 200) {
            // console.log("=200");
            // console.log(resp.data);
            _this.BBSchoolTopics = resp.data;
          } else {
            _this.$message({ type: "error", message: "数据加载失败!" });
          }
        },
