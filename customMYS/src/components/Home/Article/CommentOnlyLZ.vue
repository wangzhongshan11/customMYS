<template>
  <div>
    <div v-for="(item,index) in totalCommentList" :key="index" v-show="item.uid == currentArticle.uid">
        ...
        <el-row style="font-size:15px;color:#000000;">
          ...
          <el-col :span="2" :offset="12">
            <div class="flex-div-end-wrap" style="cursor:pointer;">
              <i class="el-icon-info" @click="openCommentToCommentM(index)"></i>
          ...
          <el-col :span="2" :offset="0">
            <div class="flex-div-end-wrap subscribe-to-comment" style="cursor:pointer;">
              ...
              <span style="margin-left:5px;" @click="subscribeComment(index)">{{item.cSubscribeListCount}}</span>
            </div>
          </el-col>
        </el-row>
      </div>
...
<script>
import { deleteRequest,postRequest, getRequest, putRequest } from "../../../utils/api";
...
export default {
  mounted() {
    ...
    this.totalCommentListCount = this.currentArticle.commentList?this.currentArticle.commentList.length:"0";
    ...
    this.$nextTick(function () {
      this.totalCommentList.forEach((item, index) => {
        if (item.cSubscribeList !== null || item.cSubscribeList !== undefined) {
          if (
        ...
        deleteRequest(
          ...
          function (msg) {
            if (msg.status !== 200) {
              ...
              _this.currentArticle.commentList[index].cSubscribeList.forEach((item, index2) => {
                if (item.uid == _this.$store.state.currentUser.id) {
                  _this.currentArticle.commentList[index].cSubscribeList.splice(index2, 1);
                  _this.currentArticle.commentList[index].cSubscribeListCount = _this.currentArticle.commentList[index].cSubscribeList.length;
                }
              });
              _this.$store.dispatch("setCurrentArticle", _this.currentArticle);
            }
        ...
        postRequest(
          ...
          function (msg) {
            if (msg.status !== 200) {
              ...
              _this.currentArticle.commentList[index].cSubscribeList.push(msg.data.newCSubscribe);
